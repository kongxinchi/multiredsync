	...
	for k, v := range makeCases(8) {
		t.Run(
			k, func(t *testing.T) {
				...
				expiries := getPoolExpiries(v.pools, mutex.names[0])
				ok, err := mutex.Extend()
				if err != nil {
				...
				expiries2 := getPoolExpiries(v.pools, mutex.names[0])
	...
	for k, v := range makeCases(4) {
		t.Run(
			k, func(t *testing.T) {
				...
				mutex1 := rs.NewMutex([]string{key}, WithExpiry(time.Hour))
				err := mutex1.Lock()
				if err != nil {
					t.Fatalf("mutex lock failed: %s", err)
				}
				...
				mutex2 := rs.NewMutex([]string{key}, WithExpiry(time.Hour), WithValue(mutex1.Value()))
...
func clogPools(pools []redis.Pool, mask int, mutex *Mutex) int {
	...
	for i, pool := range pools {
		...
		_, err = conn.Set(mutex.names[0], "foobar")
		if err != nil {
			panic(err)
		}
		_ = conn.Close()
...
func countAcquiredPools(ctx context.Context, pools []redis.Pool, mutex *Mutex) int {
	n := 0
	values := getPoolValues(ctx, pools, mutex.names[0])
