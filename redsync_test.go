	...
	for k, v := range makeCases(8) {
		t.Run(
			k, func(t *testing.T) {
				...
				mutex := rs.NewMutex([]string{"test-redsync"})
				_ = mutex.Lock()
...
func newMockPoolsGoredisV7(n int) []redis.Pool {
	...
	for i := 0; i < n; i++ {
		client := goredislib_v7.NewClient(
			&goredislib_v7.Options{
				Network: "unix",
				Addr:    servers[i+offset].Socket(),
			},
		)
...
func newMockPoolsGoredisV8(n int) []redis.Pool {
	...
	for i := 0; i < n; i++ {
		client := goredislib_v8.NewClient(
			&goredislib_v8.Options{
				Network: "unix",
				Addr:    servers[i+offset].Socket(),
			},
		)
...
func newMockPoolsGoredisV9(n int) []redis.Pool {
	...
	for i := 0; i < n; i++ {
		client := goredislib_v9.NewClient(
			&goredislib_v9.Options{
				Network: "unix",
				Addr:    servers[i+offset].Socket(),
			},
		)
...
func newMockPoolsRueidis(n int) []redis.Pool {
	...
	for i := 0; i < n; i++ {
		client, err := rueidislib.NewClient(
			rueidislib.ClientOption{
